...
SELECT schema
  ...
  ELSE 'Unexpected relkind ' || kind
    ...
    , (SELECT CASE WHEN row_estimate = 0 THEN '-'
        ELSE pg_size_pretty(ceil(total_bytes / row_estimate)::bigint) || '/row' END) AS "density"
  FROM (
  ...
  WHERE oid = parent AND schema NOT IN ('information_schema', 'repack', 'aiven_extras')
      AND schema NOT LIKE 'aws_%' AND schema NOT LIKE 'pg_%'
) a
ORDER BY total_bytes DESC, schema, name;